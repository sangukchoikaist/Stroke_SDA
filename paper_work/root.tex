...
Accurate prediction of gait phase is a crucial component in the control of wearable robots for gait assistance. Real-time recognition of a wearer’s gait phase enables the robot to synchronize its actions with the human movement, thereby enhancing both the control performance and the safety of locomotion. For instance, precise gait phase detection allows the robot to provide assistive torque during appropriate gait phases or adjust impedance parameters dynamically to accommodate varying biomechanical demands. Consequently, gait phase estimation has emerged as a key research challenge for wearable exoskeletons.

Especially in wearable robots designed for individuals with gait impairments (e.g., post-stroke, cerebral palsy, or Parkinson's disease), accurate gait phase estimation remains a significant challenge. This difficulty arises from pathological gait patterns, which often involve asymmetric motion, inconsistent cadence, and compensatory strategies that obscure clear gait phase transitions.

Conventional gait phase estimation methods adopt heuristic approaches using sensors such as inertial measurement units (IMUs) and force-sensitive resistors (FSRs) mounted on the lower limbs. For example, Ding et al. estimated stride timing by detecting the instance of maximum hip flexion using thigh-mounted IMUs. The stride time was then calculated as the interval between two consecutive peak flexion events. While such rule-based methods can be effective under constrained conditions, their generalizability to real-world settings remains limited. To address these challenges, recent studies have turned to data-driven approaches based on deep learning.

To overcome these limitations, recent studies have explored deep learning-based approaches as powerful tools for extracting and predicting gait phase. Unlike traditional rule-based or signal processing techniques, deep learning models such as Convolutional Neural Networks (CNNs), Long Short-Term Memory (LSTM) networks, and Transformers have demonstrated superior performance by capturing complex nonlinear patterns from sensor data. For instance, Hannink et al.~\cite{hannink2017cnn} accurately detected gait events using a CNN-LSTM model trained on wearable IMU data. Yan et al.~\cite{yan2020lstm} further enhanced prediction performance by integrating IMU and EMG signals with an LSTM-based framework. These studies highlight the effectiveness of deep models in fusing multimodal signals for robust, real-time gait phase recognition.

However, the performance of deep learning models is heavily influenced by the size and diversity of training datasets ~\cite{?}. Large-scale physiological datasets collected from healthy individuals, such as HuGaDB, PAMAP2, and the OU-ISIR Gait Database, have accelerated model development and validation. Nevertheless, their utility is limited when applied to individuals with gait impairments. Stroke survivors, for example, exhibit high inter-subject variability in motor patterns due to neuromuscular deficits and pathological gait compensations such as hip hiking, circumduction, and vaulting. These compensatory behaviors introduce substantial heterogeneity, making model generalization particularly challenging. Furthermore, ethical and safety concerns, including fall risk and fatigue, restrict data collection from impaired populations, resulting in small and imbalanced datasets.
