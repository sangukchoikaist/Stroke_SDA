...
Accurate estimation of gait cycle is essential for synchronizing wearable robot control with human movement during walking. While deep learning approaches have demonstrated superior performance in modeling gait dynamics from multimodal sensor signals, their generalizability to impaired populations remains limited due to inter-subject variability, pathological movement patterns, and compensatory strategies such as hip hiking and circumduction. Furthermore, safety and ethical constraints hinder the collection of extensive data from impaired individuals, posing challenges for training robust models. To overcome these limitations, we propose a supervised domain adaptation (SDA) framework for personalized gait phase estimation. Our method jointly trains a feature extractor and domain-specific regression heads using labeled data from both healthy individuals (source domain) and stroke survivors (target domain), encouraging feature alignment while preserving task-specific accuracy. Experimental results on a combined dataset, including post-stroke individuals, demonstrate that the proposed approach outperforms other baselines, achieving high estimation accuracy even with limited target data. This work contributes to the development of adaptive and data-efficient control strategies for wearable robots targeting individuals with gait impairments.
...
Accurate prediction of gait cycle is a crucial component in the control of wearable robots for gait assistance during walking. Real-time recognition of a wearer’s gait cycle enables the robot to synchronize its actions with the human movement, thereby enhancing both the control performance and the safety of locomotion. For instance, precise cycle detection allows the robot to provide assistive torque during appropriate percentage of gait cycle or adjust impedance parameters dynamically to accommodate varying biomechanical demands[ref]. Consequently, gait phase estimation has emerged as a key research challenge in the development of intelligent control strategies for wearable exoskeletons.

Conventional gait phase estimation methods adopt heuristic approaches using sensors such as inertial measurement units (IMUs) and force-sensitive resistors (FSRs) mounted on the lower limbs. For example, Ding et al. estimated stride timing by detecting the instance of maximum hip flexion using thigh-mounted IMUs. The stride time was then calculated as the interval between two consecutive peak flexion events. However, this method is only applicable in treadmill walking conditions, where stride duration is nearly constant, and fails to generalize under variable-speed or overground walking scenarios. 

To overcome these limitations, recent studies have explored deep learning-based approaches as powerful tools for extracting and predicting human gait information. Unlike traditional rule-based or signal processing techniques, deep learning models such as Convolutional Neural Networks (CNNs), Long Short-Term Memory (LSTM) networks, and Transformers have demonstrated superior performance by capturing complex nonlinear patterns from sensor data. For instance, Hannink et al.~\cite{hannink2017cnn} accurately detected gait events using a CNN-LSTM model trained on wearable IMU data. Yan et al.~\cite{yan2020lstm} further enhanced prediction performance by integrating IMU and EMG signals with an LSTM-based framework. These studies highlight the effectiveness of deep models in fusing multimodal signals for robust, real-time gait phase recognition.

However, the performance of deep learning models is heavily influenced by the size and diversity of training datasets. Large-scale physiological datasets collected from healthy individuals, such as HuGaDB, PAMAP2, and the OU-ISIR Gait Database, have accelerated model development and validation. Nevertheless, their utility is limited when applied to individuals with gait impairments. Stroke survivors, for example, exhibit high inter-subject variability in motor patterns due to neuromuscular deficits and pathological gait compensations such as hip hiking, circumduction, and vaulting. These compensatory behaviors introduce substantial heterogeneity, making model generalization particularly challenging. Furthermore, ethical and safety concerns, including fall risk and fatigue, restrict data collection from impaired populations, resulting in small and imbalanced datasets.

One promising solution to this challenge is the application of transfer learning, a technique that enables a model trained on a source domain (e.g., healthy gait data) to adapt effectively to a target domain (e.g., impaired gait data) with limited labeled samples. Transfer learning has been successfully applied in human activity recognition~\cite{zhao2019harl}, EMG decoding~\cite{mou2021transferEMG}, and medical time-series analysis~\cite{rajpurkar2017chexnet}.

In this study, we adopt a supervised domain adaptation strategy, which assumes access to labeled data in both the source and target domains. This approach is well suited for gait applications where a small number of labeled impaired samples can be collected for fine-tuning. By leveraging shared representations between domains, supervised adaptation can bridge the distribution gap while maintaining prediction accuracy.

The primary contribution of this study is the development of a deep learning framework for gait phase estimation tailored to individuals with gait impairments, using a limited amount of experimental data. We demonstrate that a model pre-trained on healthy gait data can be successfully adapted to stroke survivors through supervised domain adaptation, enabling accurate, real-time gait phase prediction. This work provides new insights into the feasibility of applying deep learning techniques to impaired gait analysis and contributes to the advancement of personalized wearable robot control for rehabilitation and assistance.

% \section{Related Works}
% \subsection{Real-time Gait Phase Estimation}
% \subsection{Real-time Gait Phase Estimation for Post-Stroke}
...
\section{Gait Differences between Non-disabled and Post-stroke }
...
Post-stroke hemiplegic gait typically presents greater impairments in distal joints compared to proximal ones [?]. To minimize the domain gap between healthy individuals and post-stroke patients, this study utilizes IMU data collected from the trunk and thigh, a proximal joint.

The input features include raw 3-axis acceleration and angular velocity signals from the thigh-mounted IMU, as well as the estimated thigh angle and angular velocity in the sagittal plane. The angle is estimated using a complementary filtering approach that combines accelerometer- and gyroscope-based estimates. Specifically, the angle from the accelerometer, $\hat{\theta}_a$, is computed from the arctangent of two acceleration components, while the gyroscope-based angle, $\hat{\theta}_g$, is obtained via numerical integration of angular velocity.
...
\end{equation}
The cutoff frequency of the complementary filter is set to $0.5,\mathrm{Hz}$, thereby emphasizing the gyroscope component over the accelerometer.
...
A total of eight sensor-derived features are used as inputs to the neural network, structured into a 2-dimensional matrix using a sliding time window. Each input window spans 200 ms (20 samples at 100 Hz sampling rate), capturing temporal dependencies within the gait cycle.
...
We formulate gait phase estimation as a regression task that maps multivariate time-series sensor input $\mathbf{x} \in \mathbb{R}^{T \times F}$ to a continuous representation of gait phase. To handle the discontinuity inherent at the gait cycle boundary, we adopt a phase portrait representation~\cite{kang2019real}. The gait phase $\phi$ is converted into angle and angular velocity as expressed as $\theta = cos(2\pi\phi)$ and $\dot{\theta} = sin(2\pi\phi)$, respectively. Therefore, the output of the gait phase estimation model is derived as 
\begin{equation}
    y = \begin{bmatrix}
        \theta \\ \dot{\theta}
    \end{bmatrix}
...
As future work, we plan to (1) integrate the proposed gait phase estimation model into the real-time control loop of a hip-assistive exoskeleton to enable phase-synchronized assistance, and (2) extend the framework to other types of pathological gait beyond stroke, such as those caused by cerebral palsy or Parkinson’s disease, to evaluate its generalizability and clinical applicability.
